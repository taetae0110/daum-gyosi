...
## 밀스톤 1.5 — 내 폰에 무료로 설치 (사이드로딩)

$99 없이 본인 폰에만 설치하는 경로. **7일마다 재서명 필요, 배포 불가** — 개인 테스트용.

1. Actions → `sideload` → Run workflow → 끝나면 아티팩트 `DaumGyosi-unsigned` 다운로드 (압축 풀면 .ipa)
2. [Sideloadly](https://sideloadly.io) 설치 (Windows), 아이폰 USB 연결
3. Sideloadly에 .ipa 드래그 → **본인 애플 ID 입력**(2단계 인증이면 앱 암호 권장) → Start
4. 아이폰 설정 → 개인정보 보호 및 보안 → **개발자 모드** 켜기(재부팅) → 설정 → 일반 → VPN 및 기기 관리에서 본인 계정 신뢰
5. 7일 뒤 만료되면 2~3번만 반복
